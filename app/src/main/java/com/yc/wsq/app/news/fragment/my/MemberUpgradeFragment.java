...
public class MemberUpgradeFragment extends BaseFragment<RechargeView, UserPresenter<RechargeView>> implements RechargeView{
    ...
    private void onIsVip(){
        DialogTools.showDialog(getActivity(), "确定", "提示", "您已经是VIP会员了！", new DialogInterface.OnClickListener() {
            @Override
    ...
    private void onSelctorInviteCode(){
        ...
        if (inviteCode.equals("0")){
            DialogTools.showDialog(getActivity(), "填写", "下次吧", "提示",
...
//
    ...
    private void onRecharge(){
        ...
        try {
            ...
            param.put(ResponseKey.getInstace().first_leader, SharedTools.getInstance(getActivity()).onGetString(ResponseKey.getInstace().first_leader));
    ...
    private void onRechargeResult(Map<String, String >result){
        ...
        if (TextUtils.equals(resultStatus, "9000")) {
            ...
            SharedTools.getInstance(getActivity()).onPutData(ResponseKey.getInstace().is_vip, "1");
            onUpdatePage();
        } else {
