package com.yc.wsq.app.news.okhttp;
...
class RequestUtil {
    ...
    RequestUtil(String methodType, String url, Map<String, String> paramsMap, File file, String fileKey, String fileType, Map<String, String> headerMap, CallBackUtil callBack) {
        this(methodType,url,null,file,null,null,fileKey,null,fileType,paramsMap,headerMap,callBack);
    }
    RequestUtil(String methodType, String url, Map<String, String> paramsMap, List<File> fileList, String fileKey, String fileType, Map<String, String> headerMap, CallBackUtil callBack) {
        this(methodType,url,null,null,null,fileList,fileKey,null,fileType,paramsMap,headerMap,callBack);
    }
    RequestUtil(String methodType, String url, Map<String, String> paramsMap, Map<String, File> fileMap, String fileType, Map<String, String> headerMap, CallBackUtil callBack) {
        this(methodType,url,null,null,null,null,null,fileMap,fileType,paramsMap,headerMap,callBack);
    }

    /**
     * 新增接口
     * @param methodType  方法类型  GET  POST
     * @param url   地址
     * @param paramsMap  参数
     * @param listFileMap  文件 包含文件  文件类型  文件key
     * @param callBack
     */
    RequestUtil(String methodType, String url, Map<String, String> paramsMap, List<Map<String, Object>> listFileMap , CallBackUtil callBack) {
        this(methodType,url,null,null,listFileMap,null,null,null,null, paramsMap, null, callBack);
    }


    private RequestUtil(String methodType, String url, String jsonStr ,File file ,List<Map<String, Object>> listFileMap, List<File> fileList,String fileKey , Map<String,File> fileMap,String fileType,Map<String, String> paramsMap,Map<String, String> headerMap, CallBackUtil callBack) {
        mMetyodType = methodType;
        mUrl = url;
        mJsonStr = jsonStr;
        mFile =file;
        mListFileMap = listFileMap;
        mfileList =fileList;
        mfileKey =fileKey;
        mfileMap =fileMap;
        mFileType =fileType;
    ...
    private void getInstance(){
        ...
        if(mFile != null || mfileList != null || mfileMap != null || mListFileMap != null){//先判断是否有文件，
            setFile();
        }else {
            //设置参数
            switch (mMetyodType){
                case com.wsq.library.okhttp.OkhttpUtil.METHOD_GET:
                    setGetParams();
                    break;
                case com.wsq.library.okhttp.OkhttpUtil.METHOD_POST:
                    mRequestBuilder.post(getRequestBody());
                    break;
                case com.wsq.library.okhttp.OkhttpUtil.METHOD_PUT:
    ...
    private void setFile() {
        ...
        }else if(mListFileMap != null){ //多个文件  对应多中文件类型
            setPostParameAndListMapFile();
        }
     ...
     * 文件集合带有文件  文件类型 文件
     */
    private void setPostParameAndListMapFile() {
        if(mListFileMap != null){
            MultipartBody.Builder builder = new MultipartBody.Builder();
            builder.setType(MultipartBody.FORM);
            if(mParamsMap != null) {
                for (String key : mParamsMap.keySet()) {
            ...
            for (Map<String, Object> map : mListFileMap){
//                String fileName = map.get("fileName").toString();
//
                String fileType = map.get("fileType").toString();
//                String fileKey = map.get()
                Iterator<Map.Entry<String, Object>> it = map.entrySet().iterator();
//                while (it.hasNext()){
//                    Map.Entry<String, Object> entry = it.next();
//                    if (entry.getKey().startsWith(ResponseKey.IMGS) || entry.getKey().equals(ResponseKey.FILE)){
